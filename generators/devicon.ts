...
const OutputPath = path.join(__dirname, '..', 'packages', 'devicon')
const IndexPath = path.join(OutputPath, 'index.ts')
const IconsPath = path.join(OutputPath, 'icons')
const SvgPath = path.join(__dirname, 'devicon_svg')
const IconsUrl = 'https://github.com/konpa/devicon/archive/master.zip'
const ArchivePath = path.join(__dirname, '..', 'assets', `devicon${getHash('https://github.com/konpa/devicon/')}.zip`)
...
function unzip (path: string): void {
  ...
  zipUtil.extractEntryTo('devicon-master/icons/', SvgPath, false, false)
}
