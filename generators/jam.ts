...
const OutputPath = path.join(__dirname, '..', 'packages', 'jam')
const IndexPath = path.join(OutputPath, 'index.ts')
const IconsPath = path.join(OutputPath, 'icons')
const SvgPath = path.join(__dirname, 'jam_svg')
const IconsUrl = 'https://github.com/michaelampr/jam/archive/master.zip'
const ArchivePath = path.join(__dirname, '..', 'assets', `jam${getHash('https://github.com/michaelampr/jam')}.zip`)
...
function unzip (path: string): void {
  ...
  zipUtil.extractEntryTo('jam-master/svg/', SvgPath, false, false)
}
